...
import NotificationPreferenceManager from './preference-manager';
...
class NotificationService {
  ...
  public async sendNotification(
    ...
    try {
      ...
      const channelHandler = this.getChannelForType(channel);
  ...
  public async processScheduledNotifications(): Promise<number> {
    try {
      ...
      for (const notification of notifications) {
        try {
          // Get channel handler for specified channel
          const channelHandler = this.getChannelForType(notification.channel);
  ...
  public async updateUserNotificationPreferences(
    userId: string,
    preferences: { notificationType: string; channel: string; enabled: boolean }[]
  ): Promise<boolean> {
    // Validate input parameters
  ...
  /**
   * Gets the appropriate notification channel implementation for a channel type
   * @param channel - Notification channel type
   * @returns Channel implementation
   */
  public getChannelForType(channel: NotificationChannel): any {
    switch (channel) {
      ...
      default:
        ...
        throw new UnsupportedChannelError(`Unsupported notification channel: ${channel}`);
    }
  }
...
export const getChannelForType = notificationService.getChannelForType.bind(notificationService);